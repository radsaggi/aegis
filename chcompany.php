...
<div id="form_container">
    ...
    $query = "SELECT * FROM `Companies` WHERE `Company ID` = '$id' ";
    ...
    $ch_data = mysqli_fetch_array($query);
    if (!isset($ch_data)) {
        ?>
        <center>Select a company first</center>
        <form method="post" action="<?php echo $_SESSION["redirect page"]; ?>">
    ...
    $ch_data["First Name"] = "";
    $ch_data["Last Name"] = "";
    if (isset($ch_data["Contact Name"])) {
        if (str_word_count($ch_data["Contact Name"]) > 1) {
            $user_name = explode(" ", $ch_data["Contact Name"]);
            $ch_data["First Name"] = $user_name[0];
            $ch_data["Last Name"] = $user_name[1];
        } else {
            $ch_data["First Name"] = $ch_data["Contact Name"];
        }
    }
    ?>

    <h1><a>Change Company Details</a></h1>
    <form id="form_696599" class="appnitro"  method="post" action="<?php echo "{$_SESSION["redirect page"]}" ?>">
        ...
        <ul >
            ...
            <li id="li_1" >
                ...
                <div>
                    <input id="element_1" name="CompanyName" class="element text medium" type="text" maxlength="255" value="<?php echo $ch_data["Company Name"]; ?>"/> 
                </div> 
            </li>		
            <li id="li_3" >
                <label class="description" for="element_3">Address </label>
                <div>
                    <textarea id="element_3" name="Address" class="element textarea medium"><?php echo $ch_data["Address"]; ?></textarea> 
                </div> 
            </li>		
            <li id="li_4" >
                <label class="description" for="element_4">Contact Name </label>
                <span>
                    <input id="element_4_1" name= "ContactFirstName" class="element text" maxlength="255" size="8" value="<?php echo $ch_data["First Name"]; ?>"/>
                    <label>First</label>
                </span>
                <span>
                    <input id="element_4_2" name= "ContactLastName" class="element text" maxlength="255" size="14" value="<?php echo $ch_data["Last Name"]; ?>"/>
                    <label>Last</label>
                </span> 
            </li>		
            <li id="li_5" >
                ...
                <div>
                    <input id="element_5" name="ContactNumber" class="element text medium" type="text" maxlength="255" value="<?php echo $ch_data["Contact Number"]; ?>"/> 
                </div> 
            </li>		
            <li id="li_7" >
                <label class="description" for="element_7">Contact Designation </label>
                <div>
                    <input id="element_7" name="ContactDesignation" class="element text medium" type="text" maxlength="255" value="<?php echo $ch_data["Contact Designation"]; ?>"/> 
                </div> 
            </li>		
            <li id="li_10" >
                ...
                <div>
                    <select id="element_10" name="SCategory">
                        ...
                        while ($sponsorship_category_data) {
                            $option_cmd = "<option value=\"%s\" %s> %s </option>";
                            if ($ch_data["Sponsorship Category"] == $sponsorship_category_data["Category ID"]) {
                                $option_cmd = sprintf($option_cmd, $sponsorship_category_data["Category ID"], "selected", $sponsorship_category_data["Category Name"]);
                            } else {
            ...
            <li id="li_13" >
                ...
                <div>
                    <input id="element_13" name="SponsorshipFor" class="element textarea medium" value="<?php echo $ch_data["Sponsorship For"]; ?>">
                </div> 
            </li>
            <li id="li_14" >
                ...
                <div>
                    <select id="element_14" name="ProbabilityIndex">
                        ...
                        while ($sponsorship_category_data) {
                            $option_cmd = "<option value=\"%s\" %s> %s </option>";
                            if ($ch_data["Probability Index ID"] == $sponsorship_category_data["Probability Index ID"]) {
                                $option_cmd = sprintf($option_cmd, $sponsorship_category_data["Probability Index ID"], "selected", $sponsorship_category_data["Description"]);
                            } else {
            ...
            <li class="buttons">
                <input type="hidden" name="set" value="<?php echo $PAGES["Change Company"] ?>" />
                <input type="hidden" name="coid" value="<?php echo $id ?>" />
